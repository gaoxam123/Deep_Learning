...
class Discriminator(nn.Module):
    def __init__(self, channels_img, features_d, num_classes, img_size): # N * channels_img * 64 * 64
        super().__init__()
    ...
    def forward(self, x, labels):
        embedding = self.embed(labels).reshape(labels.shape[0], 1, self.img_size, self.img_size)
...
class Generator(nn.Module):
    def __init__(self, z_dim, channels_img, features_g, num_classes, img_size, embed_size):
        ...
        self.gen = nn.Sequential(
            ...
            self.block(z_dim + embed_size, features_g * 16, kernel_size=4, stride=1, padding=0), # N x features_g * 16 x 4 x 4
